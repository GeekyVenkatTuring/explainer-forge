#!/usr/bin/env python3
"""Lagot / loquat farmer course — chapter-wise build orchestrator (Remotion).

Each chapter renders to its own MP4 and all chapters concat into the master.
Burned-in captions ON (the user asked for subtitles). English edge-tts
(en-IN-NeerjaNeural). 16:9 1080p30, no vertical. Target: well over 30 minutes.
...
TTS: edge-tts English (en-IN-NeerjaNeural). Idempotent —
...
PREFIX = "lag"
CAP_MAXWORDS = 8
CAP_WRAP = 50       # wrap width (chars) per caption line (mostly 1 line at 33px)
...
def run_tts():
    ...
    for ch in CHAPTERS:
        ...
        for seg in ch["segments"]:
            ...
            if d > 90:  # frozen-frame risk (skills/02,03,09) — split or develop to p≈0.85
...
def render_one(ch, attempts=3):
    """Render one chapter at concurrency 8, delivering the MP4 immediately. Retries on
    transient headless-browser crashes ("Session closed" / frame timeout) instead of
    aborting the whole run. Returns the out path on success, None on repeated failure."""
    props = os.path.join(ART, f'{ch["id"]}.json')
    ...
    for a in range(1, attempts + 1):
        ...
        if os.path.exists(out):
            os.remove(out)  # drop any partial file so a retry starts clean
        time.sleep(5)
