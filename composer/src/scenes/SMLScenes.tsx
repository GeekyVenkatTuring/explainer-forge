...
// ---------------------------------------------------------------- identity
const T = makeTheme({ accent: "#14B8A6", bg0: "#05060F", bg1: "#0A0C1A", bg2: "#11142A", panel: "#161A32" });
const A = {
  graph: "#2DD4BF", ont: "#FBBF24", kg: "#22D3EE", twin: "#A78BFA",
...
const SOURCES = [
  { emoji: "📈", label: "NSE / BSE", sub: "live & EOD prices", c: A.kg },
  { emoji: "📊", label: "screener.in", sub: "P/E · fundamentals", c: A.ont },
  { emoji: "📑", label: "BSE filings", sub: "quarterly results", c: A.eng },
  { emoji: "📰", label: "News RSS", sub: "Google · Moneycontrol", c: A.agent },
  { emoji: "🎤", label: "Concalls", sub: "guidance · roadmap", c: A.twin },
  { emoji: "⚡", label: "Broker API", sub: "real-time ticks", c: A.bad },
];
  ...
  return (
    <AbsoluteFill style={{ alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ textAlign: "center", transform: `scale(${0.93 + pop(0) * 0.07})` }}>
        ...
        <div style={{ fontFamily: SANS, fontWeight: 800, fontSize: 118, lineHeight: 1.0, letterSpacing: -4, color: T.text }}>
          <div>The Factual</div>
          <div style={{ color: A.graph, textShadow: `0 0 70px ${mix(T.bg0, A.graph, 0.7)}` }}>Stock Agent</div>
        </div>
...
const SilosScene: React.FC<{ dur?: number; foot?: string }> = ({ dur, foot }) => {
  ...
  return (
    <Stage>
      <Head theme={T} kicker="THE PROBLEM · ONE STOCK, SIX TABS" title="The facts are scattered — and disconnected" color={A.bad} o={p(0, 0.06)} />
      {SOURCES.map((s, i) => {
      ...
      <div style={{
        ...
        <div style={{ fontFamily: SANS, fontWeight: 800, fontSize: 30, color: T.text }}>“How is TCS doing right now?”</div>
        <div style={{ fontFamily: MONO, fontSize: 21, color: A.bad, marginTop: 10 }}>= 6 tabs, 20 min, still unsure</div>
      </div>
      <Foot p={p(0.82, 0.92)}>{foot || "Six sources, no shared meaning — and no guarantee any number is current or correct."}</Foot>
    </Stage>
  );
};
  ...
  return (
    <Stage>
      ...
      <div style={{ position: "absolute", left: 150, top: 260, width: 700, opacity: p(0.08, 0.18) }}>
        ...
        {["TCS: ₹4,102.50", "P/E: 29.4", "Q3 PAT +12%", "news headline", "concall Q&A", "ISIN INE467B01029"].map((r, i) => (
      ...
      {(() => {
        const nodes = [
          { x: 1120, y: 300, l: "Company", c: A.kg }, { x: 1420, y: 300, l: "Price", c: A.agent },
          { x: 1580, y: 520, l: "P/E", c: A.ont }, { x: 1360, y: 700, l: "Q3 result", c: A.eng },
          { x: 1080, y: 560, l: "News", c: A.twin },
        ];
      ...
      <Foot p={p(0.82, 0.92)}>{foot || "Link a company to its price, filings, and news — then the agent can reason across all of it."}</Foot>
    </Stage>
  );
};
...
const IngestScene: React.FC<{ dur?: number; foot?: string }> = ({ dur, foot }) => {
  const frame = useCurrentFrame();
  const p = useP(dur);
  const connectors = ["price poll / ticks", "fundamentals parse", "filing → LLM", "RSS + sentiment", "transcript → LLM", "websocket ticks"];
  // right-side unified graph
  const g = [
    { x: 1330, y: 250, l: "Company", c: A.graph }, { x: 1590, y: 340, l: "Price", c: A.agent },
    { x: 1290, y: 430, l: "P/E", c: A.kg }, { x: 1560, y: 540, l: "Q-Result", c: A.eng },
    { x: 1310, y: 640, l: "News", c: A.twin }, { x: 1600, y: 720, l: "Guidance", c: A.bad },
    { x: 1360, y: 810, l: "Sector", c: A.ont },
  ];
  ...
  return (
    <Stage>
      ...
      <Foot p={p(0.84, 0.93)}>{foot || "Each source stays where it is; the graph links it into one company view — every fact stamped with its source."}</Foot>
    </Stage>
  );
};
...
const MergeScene: React.FC<{ dur?: number; foot?: string }> = ({ dur, foot }) => {
  ...
  const p = useP(dur);
  const dups = [
    { l: "Reliance", src: "news / screener", c: A.ont, y: 280 },
    { l: "RELIANCE.NS", src: "NSE / yfinance", c: A.eng, y: 500 },
    { l: "500325", src: "BSE scrip code", c: A.kg, y: 720 },
  ];
  ...
  return (
    <Stage>
      ...
      <div style={{ position: "absolute", left: tx - 20, top: ty - 20, width: 280, height: 132, borderRadius: 18, background: mix(T.panel, A.kg, 0.22), border: `3px solid ${A.kg}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: merged ? p(0.7, 0.78) : mp * 0.4, boxShadow: `0 0 ${36 + Math.sin(frame * 0.08) * 12}px ${mix(T.bg0, A.kg, 0.45)}` }}>
        <div style={{ fontFamily: SANS, fontWeight: 800, fontSize: 32, color: T.text }}>:Company Reliance</div>
        <div style={{ fontFamily: MONO, fontSize: 18, color: A.kg, marginTop: 6 }}>{merged ? "ISIN INE002A01018 ✓" : "resolving…"}</div>
      </div>
      {/* +34% callout */}
      ...
      <Foot p={p(0.86, 0.94)}>{foot || "Resolve on ISIN first, name second — one Reliance across every source, no double-counting."}</Foot>
    </Stage>
  );
};
...
const TelemetryScene: React.FC<{ dur?: number; foot?: string }> = ({ dur, foot }) => {
  ...
  const nodes = [
    { x: 360, y: 320, l: "RELIANCE" }, { x: 760, y: 250, l: "TCS" }, { x: 1180, y: 300, l: "HDFCBANK" },
    { x: 560, y: 560, l: "INFY" }, { x: 1000, y: 600, l: "NIFTY 50" }, { x: 1420, y: 540, l: "ITC" },
  ];
  const edges = [[0, 1], [1, 3], [1, 4], [2, 4], [0, 2], [2, 5]];
  // green = up, red = down, amber = flat (illustrative tick motion)
  const health = (i: number) => {
    const s = (Math.sin(frame * 0.03 + i * 1.3) + 1) / 2;
    return s > 0.72 ? A.ok : s > 0.4 ? A.ont : A.bad;
  };
  return (
    <Stage>
      ...
      <Foot p={p(0.82, 0.92)}>{foot || "The twin holds each name's live price and last-known fundamentals — every value stamped with its as-of time."}</Foot>
    </Stage>
  );
};
...
const CascadeScene: React.FC<{ dur?: number; foot?: string }> = ({ dur, foot }) => {
  ...
  const nodes = [
    { id: 0, x: 150, y: 470, l: "Crude +10%", t: "SHOCK", c: A.ont, depth: 0 },
    { id: 1, x: 520, y: 470, l: "Input cost ↑", t: "DRIVER", c: A.eng, depth: 1 },
    { id: 2, x: 900, y: 320, l: "Aviation", t: "SECTOR", c: A.kg, depth: 2 },
    { id: 3, x: 900, y: 620, l: "Paints", t: "SECTOR", c: A.kg, depth: 2 },
    { id: 4, x: 1330, y: 250, l: "Airline A", t: "HOLDING", c: A.agent, depth: 3 },
    { id: 5, x: 1330, y: 430, l: "Airline B", t: "HOLDING", c: A.agent, depth: 3 },
    { id: 6, x: 1330, y: 640, l: "Paint Co", t: "HOLDING", c: A.agent, depth: 3 },
  ];
  ...
  return (
    <Stage>
      <Head theme={T} kicker="DIGITAL TWIN · SIMULATION" title="“If crude jumps 10% — what's exposed?”" color={A.twin} o={p(0, 0.06)} />
      {edges.map(([a, b], i) => {
      ...
      {nodes.map((nd) => {
        ...
        return (
          <div key={nd.id} style={{ position: "absolute", left: nd.x, top: nd.y, width: 300, height: 68, borderRadius: 13, background: mix(T.panel, c, on ? 0.2 : 0.09), border: `2.5px solid ${on ? c : mix(T.line, c, 0.6)}`, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 18px", boxSizing: "border-box", opacity: p(0.06 + nd.depth * 0.05, 0.16 + nd.depth * 0.05), boxShadow: on ? `0 0 ${18 + Math.sin(frame * 0.2) * 8}px ${mix(T.bg0, c, 0.5)}` : "none" }}>
            <div style={{ fontFamily: MONO, fontSize: 15, color: c, letterSpacing: 1 }}>{nd.t}{on ? " · HIT" : ""}</div>
      ...
      <div style={{ position: "absolute", left: 0, right: 0, top: 800, textAlign: "center", opacity: p(0.72, 0.82), fontFamily: SANS, fontWeight: 800, fontSize: 28, color: A.bad }}>
        3 holdings exposed — flagged before the open
      </div>
      <Foot p={p(0.86, 0.94)}>{foot || "Illustrative: trace a macro shock along the graph's own sector→company links. Not a prediction."}</Foot>
    </Stage>
  );
};
...
const AgentLoopScene: React.FC<{ dur?: number; foot?: string }> = ({ dur, foot }) => {
  ...
  const steps = [
    ...
    { l: "ANSWER", sub: "cited, factual, no guesses", c: A.agent, x: 560, y: 620 },
  ];
...
const TraverseScene: React.FC<{ dur?: number; foot?: string }> = ({ dur, foot }) => {
  ...
  const path = [
    { x: 150, y: 250, l: "“Analyze TCS”", sub: "one question", c: A.agent },
    { x: 470, y: 250, l: "Price", sub: "₹4,102.50 · NSE · 15:29", c: A.kg },
    { x: 790, y: 250, l: "P/E", sub: "29.4 · screener.in", c: A.ont },
    { x: 790, y: 470, l: "Q3 result", sub: "PAT +12% YoY · BSE", c: A.eng },
    { x: 470, y: 470, l: "News", sub: "2 items · Moneycontrol", c: A.twin },
    { x: 150, y: 470, l: "Guidance", sub: "concall · margins steady", c: A.ont },
    { x: 470, y: 690, l: "ANSWER", sub: "grounded · every fact cited", c: A.ok },
  ];
  ...
  return (
    <Stage>
      <Head theme={T} kicker="THE PAYOFF · GROUNDED MULTI-FACTOR ANSWER" title="One question. The agent walks the graph." color={A.agent} o={p(0, 0.06)} />
      {seg.map(([a, b], i) => {
        const on = walk >= i + 0.5;
        return (
          <React.Fragment key={i}>
      ...
      <div style={{ position: "absolute", left: 1150, top: 470, width: 620, height: 300, borderRadius: 20, background: mix(T.panel, A.ok, 0.08), border: `2.5px solid ${mix(T.line, A.ok, 0.5)}`, opacity: p(0.6, 0.72), padding: "26px 32px", boxSizing: "border-box" }}>
        <div style={{ fontFamily: MONO, fontSize: 20, color: A.ok, letterSpacing: 2 }}>THE GUARANTEE</div>
        <div style={{ display: "flex", alignItems: "baseline", gap: 16, marginTop: 16 }}>
          <span style={{ fontFamily: MONO, fontWeight: 800, fontSize: 52, color: A.ok }}>0</span>
          <span style={{ fontFamily: SANS, fontSize: 24, color: T.muted }}>uncited numbers · 0 guesses</span>
        </div>
        <div style={{ fontFamily: SANS, fontSize: 23, color: T.text, marginTop: 16, lineHeight: 1.4 }}>Every figure carries <span style={{ color: A.ok, fontWeight: 800 }}>source + timestamp</span>. Missing or stale → it says so.</div>
        <div style={{ fontFamily: MONO, fontSize: 19, color: A.muted, marginTop: 14 }}>6 tabs · 20 min → 1 cited answer</div>
      </div>
      <Foot p={p(0.86, 0.94)}>{foot || "Retrieve from the graph, reason over the factors, answer with citations — never a number that isn't grounded."}</Foot>
    </Stage>
  );
};
...
const ArchitectureScene: React.FC<{ dur?: number; foot?: string }> = ({ dur, foot }) => {
  ...
  return (
    <Stage>
      <Head theme={T} kicker="THE WHOLE ARCHITECTURE" title="Sources in at the bottom, a cited answer out the top" color={A.graph} o={p(0, 0.06)} />
      ...
      <div style={{ position: "absolute", left: 1500, top: 250, width: 300, height: 110, borderRadius: 16, background: mix(T.panel, A.agent, 0.16), border: `2.5px solid ${A.agent}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: p(0.72, 0.82), boxShadow: `0 0 ${30 + Math.sin(frame * 0.07) * 12}px ${mix(T.bg0, A.agent, 0.4)}` }}>
        ...
        <div style={{ fontFamily: MONO, fontSize: 18, color: A.agent, marginTop: 6 }}>ask · analyze · cite</div>
      </div>
...
// ============================================================================ ROUTER
export const SMLScene: React.FC<{ variant: string;[key: string]: unknown }> = ({ variant, ...rest }) => {
  let content: React.ReactNode;
  let accent = A.graph;
  const r = rest as any;
  switch (variant) {
    case "sml_title": content = <TitleScene {...r} />; break;
    case "sml_divider": content = <Divider {...r} />; accent = r.color || A.graph; break;
    case "sml_stack": content = <StackScene {...r} />; break;
    case "sml_silos": content = <SilosScene {...r} />; accent = A.bad; break;
    case "sml_hook": content = <HookScene {...r} />; break;
    case "sml_triple": content = <TripleScene {...r} />; accent = A.ont; break;
    case "sml_compare": content = <CompareScene {...r} />; accent = r.color || A.graph; break;
    case "sml_cards": content = <CardsScene {...r} />; accent = r.color || A.graph; break;
    case "sml_orbit": content = <OrbitScene {...r} />; accent = r.color || A.graph; break;
    case "sml_code": content = <CodeScene {...r} />; accent = r.color || A.kg; break;
    case "sml_chart": content = <ChartScene {...r} />; accent = r.color || A.graph; break;
    case "sml_tower": content = <TowerScene {...r} />; accent = r.color || A.twin; break;
    case "sml_pipeline": content = <PipelineScene {...r} />; accent = r.color || A.graph; break;
    case "sml_ingest": content = <IngestScene {...r} />; accent = A.kg; break;
    case "sml_merge": content = <MergeScene {...r} />; accent = A.kg; break;
    case "sml_telemetry": content = <TelemetryScene {...r} />; accent = A.twin; break;
    case "sml_cascade": content = <CascadeScene {...r} />; accent = A.twin; break;
    case "sml_gnn": content = <GnnScene {...r} />; accent = A.eng; break;
    case "sml_agentloop": content = <AgentLoopScene {...r} />; accent = A.agent; break;
    case "sml_twobrain": content = <TwoBrainScene {...r} />; accent = A.agent; break;
    case "sml_traverse": content = <TraverseScene {...r} />; accent = A.agent; break;
    case "sml_architecture": content = <ArchitectureScene {...r} />; break;
    case "sml_timeline": content = <TimelineScene {...r} />; accent = r.color || A.graph; break;
    case "sml_recap": content = <RecapScene {...r} />; break;
