/**
 * IDMScenes.tsx — "INDmoney: an honest review" (prefix `idm`, English, Nova).
 *
 * India-context platform review — read skills/12-market-research.md §5 (platform video:
 * verify pricing from the platform's own dated page; every complaint sourced; unbiased).
 * Built on the same proven engine as GWScenes (the intl-investing video that named INDmoney).
 *
 * Identity (skills/04):
 *   theme accent = teal (the INDmoney brand family). Semantic accents:
 *     brand teal   #2DD4BF — INDmoney, the app, the on-ramp
 *     usd   blue   #60A5FA — US markets / the dollar / custody abroad
 *     cost  rose   #FB7185 — costs, fees, money leaving
 *     warn  amber  #F59E0B — caution, complaints, "look before you leap"
 *     ok    green  #4ADE80 — the good path, "keep using it", safety
 *     tax   violet #A78BFA — tax, rules, compliance
 *   Recurring motif: a globe with ₹ and $ orbiting it (GlobeOrbit) — title, dividers,
 *   recap. Every content scene carries a bottom scene-progress bar. Two COMPUTED heroes:
 *   the custody flow (idm_pipe) and the cost waterfall (idm_fees).
 */
...
const T = makeTheme({ accent: "#2DD4BF" });
const A = { brand: "#2DD4BF", usd: "#60A5FA", cost: "#FB7185", warn: "#F59E0B", ok: "#4ADE80", tax: "#A78BFA" };
      ...
      {Array.from({ length: 14 }).map((_, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            fontFamily: MONO, fontWeight: 800, fontSize: 24, color: rupee ? A.brand : A.usd,
            opacity: 0.12 + rnd(i, 3) * 0.2, textShadow: `0 0 12px ${rupee ? A.brand : A.usd}`,
          }}>{rupee ? "₹" : "$"}</div>
        );
      })}
    </div>
...
const Progress: React.FC<{ dur?: number }> = ({ dur }) => {
  ...
  return (
    <div style={{ position: "absolute", left: 0, bottom: 0, height: 6, width: `${p(0, 1) * 100}%`,
      background: `linear-gradient(90deg, ${A.brand}, ${A.usd})`, opacity: 0.55 }} />
  );
};
    ...
    <AbsoluteFill style={{ alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ textAlign: "center", transform: `scale(${0.92 + pop(0) * 0.08})`, zIndex: 2 }}>
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 26 }}>
          <Kicker theme={T} text="US STOCKS APP · AN HONEST REVIEW" cx />
        </div>
        <div style={{ fontFamily: SANS, fontWeight: 800, fontSize: 118, lineHeight: 1.04, letterSpacing: -3, color: T.text }}>
          <div>Is <span style={{ color: A.brand, textShadow: `0 0 60px ${mix(T.bg0, A.brand, 0.7)}` }}>INDmoney</span></div>
          <div>worth it?</div>
        </div>
        <div style={{ height: 6, width: interpolate(p(0.18, 0.45), [0, 1], [0, 620]), background: `linear-gradient(90deg, ${A.brand}, ${A.usd})`, borderRadius: 3, margin: "30px auto" }} />
        <div style={{ fontFamily: SANS, fontSize: 35, color: T.muted, opacity: p(0.28, 0.5), lineHeight: 1.4 }}>
          How it works · Is it safe · Every cost · Real complaints
        </div>
      </div>
    </AbsoluteFill>
  );
...
// ---------------------------------------------------------------- idm_routes (2-panel; verdict / overview)
const RoutesScene: React.FC<{
  dur?: number; kicker?: string; title?: string;
  left?: { label: string; sub: string; items: string[]; color?: string; emoji?: string };
  right?: { label: string; sub: string; items: string[]; color?: string; emoji?: string };
}> = ({
  dur, kicker = "TWO SIDES", title = "",
  left = { label: "", sub: "", items: [] },
  right = { label: "", sub: "", items: [] },
}) => {
  const frame = useCurrentFrame();
  const p = useP(dur);
  const cols = [
    { ...left, color: left.color || A.ok, emoji: left.emoji || "✅" },
    { ...right, color: right.color || A.warn, emoji: right.emoji || "⚠️" },
  ];
  return (
    <Stage>
      <Head theme={T} kicker={kicker} title={title} color={A.brand} o={p(0, 0.06)} />
      {cols.map((col, i) => {
        const at = 0.1 + i * 0.16;
        const o = p(at, at + 0.1);
        return (
          <div key={i} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div>
                <div style={{ fontFamily: SANS, fontWeight: 800, fontSize: 38, color: col.color, lineHeight: 1.1 }}>{col.label}</div>
                <div style={{ fontFamily: SANS, fontSize: 23, color: T.muted, marginTop: 4 }}>{col.sub}</div>
              </div>
            </div>
              ...
              {col.items.map((it, ri) => {
                ...
                return (
                  <div key={ri} style={{ display: "flex", alignItems: "flex-start", gap: 14, opacity: io }}>
                    ...
                    <span style={{ fontFamily: SANS, fontWeight: 600, fontSize: 27, color: T.text, lineHeight: 1.35 }}>{it}</span>
                  </div>
                );
              })}
            </div>
...
// ---------------------------------------------------------------- idm_pipe (bespoke: custody / money flow)
const PipeScene: React.FC<{
  dur?: number; kicker?: string; title?: string; color?: string; note?: string;
  nodes?: { emoji: string; label: string; sub: string; c: string }[];
}> = ({
  dur, kicker = "UNDER THE HOOD", title = "Who actually holds your shares?", color = A.usd,
  note = "",
  nodes = [],
}) => {
  const frame = useCurrentFrame();
  ...
  // widths tuned for up to 5 nodes across the stage
  const cfg = n <= 4 ? { w: 320, gapX: 400, x0: 130 } : { w: 280, gapX: 360, x0: 100 };
  const { w, gapX, x0 } = cfg;
  const y = 360;
  return (
    <Stage>
      ...
      {nodes.map((it, i) => {
        const at = 0.08 + i * (0.55 / Math.max(1, n));
        ...
        return (
          <React.Fragment key={i}>
            {i > 0 && (
              <>
                <Wire x1={x0 + (i - 1) * gapX + w} y1={y + 115} x2={x - 8} y2={y + 115} p={p(at - 0.05, at)} color={it.c} w={3} />
                <Flow x1={x0 + (i - 1) * gapX + w} y1={y + 115} x2={x - 8} y2={y + 115} color={it.c} n={5} o={p(at, at + 0.1)} />
              </>
            )}
            <div style={{ position: "absolute", left: x, top: y, width: w, height: 250,
              borderRadius: 18, boxSizing: "border-box", padding: "24px 18px", textAlign: "center",
              background: mix(T.panel, it.c, o > 0.5 ? (active ? 0.16 : 0.09) : 0.02),
              border: `2.5px solid ${o > 0.5 ? mix(T.bg2, it.c, active ? 1 : 0.7) : T.bg2}`,
              opacity: o, transform: `translateY(${(1 - o) * 20}px) scale(${active ? 1.04 : 1})` }}>
              <div style={{ fontSize: 50 }}>{it.emoji}</div>
              <div style={{ fontFamily: SANS, fontWeight: 800, fontSize: 26, color: it.c, marginTop: 10, lineHeight: 1.2 }}>{it.label}</div>
              <div style={{ fontFamily: SANS, fontSize: 22, color: T.muted, marginTop: 8, lineHeight: 1.32 }}>{it.sub}</div>
            </div>
          </React.Fragment>
        );
      })}
      {note && (
        <div style={{ position: "absolute", left: 150, top: 800, width: 1620, textAlign: "center", opacity: p(0.72, 0.88) }}>
...
// ---------------------------------------------------------------- idm_fees (bespoke: cost waterfall)
const FeeStackScene: React.FC<{
  ...
  dur, kicker = "WHAT IT COSTS", title = "Add ₹1,00,000 — what actually gets invested?", gross = 100000,
  steps = [],
  note = "",
}) => {
  const frame = useCurrentFrame();
  const p = useP(dur);
  let bal = gross;
  const bars = [{ label: "You add", val: gross, c: A.brand, delta: gross, note: "in rupees" } as { label: string; val: number; c: string; delta: number; note?: string }];
        ...
        return (
          <React.Fragment key={i}>
            ...
            <div style={{ position: "absolute", left: x - 10, top: baseY - h - 46, width: bw + 20, textAlign: "center", opacity: o }}>
              <Counter p={p(at + 0.02, at + 0.14)} to={b.val} prefix="₹" color={isNet ? A.ok : b.c} size={30} comma />
            </div>
...
// ---------------------------------------------------------------- idm_steps (generic pipeline)
const StepsScene: React.FC<{
  dur?: number; kicker?: string; title?: string; color?: string; note?: string;
  items?: { emoji: string; label: string; sub: string; c?: string }[];
}> = ({ dur, kicker = "HOW IT WORKS", title = "", color = A.brand, note = "", items = [] }) => {
  const frame = useCurrentFrame();
  const p = useP(dur);
...
}> = ({ dur, kicker = "", title = "", color = A.brand, items = [] }) => {
  const frame = useCurrentFrame();
  const p = useP(dur);
        ...
        return (
          <div key={i} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
              ...
              <span style={{ fontFamily: SANS, fontWeight: 800, fontSize: h > 250 ? 33 : 28, color, lineHeight: 1.22 }}>{it.k}</span>
            </div>
            <div style={{ fontFamily: SANS, fontSize: h > 250 ? 26 : 24, color: T.text, marginTop: 16, lineHeight: 1.4, opacity: 0.55 + o * 0.45 }}>{it.v}</div>
            {it.chip && (
              <div style={{ position: "absolute", right: 24, bottom: 20, fontFamily: MONO, fontWeight: 700, fontSize: 21,
...
}> = ({ dur, kicker = "BY THE NUMBERS", title = "", note = "", color = A.brand, stats = [] }) => {
  const frame = useCurrentFrame();
  const p = useP(dur);
        ...
        return (
          <div key={i} style={{
            ...
            <div style={{ marginTop: 18 }}>
              <Counter p={p(at + 0.04, at + 0.22)} to={s.to} prefix={s.prefix || ""} suffix={s.suffix || ""} decimals={s.decimals || 0} color={c} size={n === 2 ? 80 : 62} comma />
            </div>
...
const DividerScene: React.FC<{ dur?: number; n?: number; title?: string; sub?: string; color?: string }> = ({
  dur, n = 1, title = "", sub = "", color = A.brand,
}) => {
  const frame = useCurrentFrame();
  const p = useP(dur);
  return (
...
const RecapScene: React.FC<{ dur?: number; items?: string[]; closer?: string; title?: string }> = ({
  dur, items = [], closer = "", title = "The verdict in one breath",
}) => {
  const frame = useCurrentFrame();
  const p = useP(dur);
  return (
    <AbsoluteFill style={{ padding: "54px 130px", justifyContent: "center" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 11, maxWidth: 1460, margin: "0 auto", width: "100%", zIndex: 2 }}>
        {items.map((it, i) => {
          ...
          const accent = [A.brand, A.usd, A.cost, A.warn, A.ok, A.tax][i % 6];
          return (
      ...
      <div style={{ textAlign: "center", marginTop: 26, opacity: p(0.8, 0.9), zIndex: 2 }}>
        <div style={{ fontFamily: SANS, fontWeight: 800, fontStyle: "italic", fontSize: 36, color: A.ok, textShadow: `0 0 ${28 + Math.sin(frame * 0.06) * 14}px ${mix(T.bg0, A.ok, 0.7)}` }}>{closer}</div>
      </div>
    </AbsoluteFill>
  );
};
...
// ===========================================================================
export const IDMScene: React.FC<{ variant: string;[key: string]: unknown }> = ({ variant, ...rest }) => {
  let content: React.ReactNode;
  let accent = A.brand;
  switch (variant) {
    case "idm_title": content = <TitleScene {...(rest as any)} />; break;
    case "idm_routes": content = <RoutesScene {...(rest as any)} />; accent = A.brand; break;
    case "idm_pipe": content = <PipeScene {...(rest as any)} />; accent = (rest as any).color || A.usd; break;
    case "idm_fees": content = <FeeStackScene {...(rest as any)} />; accent = A.cost; break;
    case "idm_steps": content = <StepsScene {...(rest as any)} />; accent = (rest as any).color || A.brand; break;
    case "idm_cards": content = <CardsScene {...(rest as any)} />; accent = (rest as any).color || A.brand; break;
    case "idm_stats": content = <StatsScene {...(rest as any)} />; accent = (rest as any).color || A.brand; break;
    case "idm_compare": content = <CompareScene {...(rest as any)} />; accent = (rest as any).color || A.brand; break;
    case "idm_divider": content = <DividerScene {...(rest as any)} />; accent = (rest as any).color || A.brand; break;
    case "idm_recap": content = <RecapScene {...(rest as any)} />; accent = A.ok; break;
